...
# CODE HERE
transactions_tbl %>%
  plot_time_series(.date_var = purchased_at, .value = revenue)
#Time frequency is weekly.
```
...
```{r}
# CODE HERE
product_pages_weekly_tbl <- google_analytics_by_page_tbl %>%
...
transactions_product_page_sessions_weekly_tbl <- transactions_weekly_tbl %>%
  left_join(product_pages_weekly_tbl, by = c("purchased_at" = "date") )
```
...
```{r}
# CODE HERE
plot_missing(transactions_product_page_sessions_weekly_tbl)
```
...
# page visits go up as revenue goes up for about 2 weeks(2 lags)

- What about the relationship between Jumpstart, Learning Labs PRO page visits and Revenue?
# page visits go up as revenue goes up for a much longer time of up to 11-12 weeks(11-12 lags)
