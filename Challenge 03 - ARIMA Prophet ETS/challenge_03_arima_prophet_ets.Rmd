...
workflow_fit_lm_2_lag    <- challenge_02_artifacts$trained_models$workflow_fit_lm_2_lag
...
model_fit_1_arima_basic <- arima_reg() %>%
  ...
  fit(revenue ~ purchased_at, data = train_tbl)
...
- Has the AIC improved? 
  Yes AIC went from 183.41 to 128.99
- What coefficients do you see in your model?
  1 autoregressive coefficient, 2 lagged error coefficients, 2 event coefficents
autoregressive term, two lagged error terms and 
- Is this a seasonal model?
  no
...
model_fit_3_arima_sarimax <- arima_reg(
  ...
  set_engine("auto_arima") %>%
  fit(revenue ~ purchased_at + event_november_sale + event_product_launch, data = train_tbl)
...
- Which ARIMA forecasts performed the best on the test data set?
   ARIMA with external regressors and fourier series
- Which ARIMA forecasts had the lowest MAE / RMSE?
   The ARIMA with external regressors and fourier terms
- Which ARIMA forecasts had the highest variance explained
    The ARIMA with external regressors and fourier terms
- How did the forecasts do predicting the global trend?
  Only the ARIMA with external regressors caught the global trend
- How did the forecasts do predicting the local trend?
  - The SARIMAX and ARIMA with external regressors seemed to be best at local trend with SARIMAX being the best
...
```{r}
# CODE HERE
calibration_tbl_prophet <- model_tbl_prophet %>%
  modeltime_calibrate(new_data = test_tbl)

calibration_tbl_prophet
```
...
- Which Prophet forecasts performed the best on the test data set?
   The prophet model with external regressors and fourier series term
- Which Prophet forecasts had the lowest MAE / RMSE?
   Prophet model with external regressors and fourier series
- Which Prophet forecasts had the highest variance explained
  - Prophet model with external regressors and fourier series
- How did the forecasts do predicting the global trend?
 - They all had an upward trend
- How did the forecasts do predicting the local trend?
 - None of them did very well with the local trend
```{r}
# Checkpoint data
...
- How does the AIC compare to the AIC of the ARIMA model?
  It is almost double ARIMA model
...
- How did the forecasts do predicting the global trend?
  ETS predicted well
- How did the forecasts do predicting the local trend?
  Neither captured local trend. TBATS did better at this.
...
refit_tbl %>%
  ...
  mutate(
    across(
      .cols = .value:.conf_hi,
      .fns  = ~standardize_inv_vec(., std_mean, std_sd)
    )
  ) %>%
  mutate(
    across(
