...
future_tbl %>% filter(is.na(pageViews_lag28))

# 2.0 TIME SPLIT ----

splits <- data_prepared_tbl %>% time_series_split(date_var = date, assess = 28, cumulative = TRUE)
...
wflw_fit_xgboost <- workflow() %>%
  add_model(boost_tree() %>% set_mode("regression") %>% set_engine("xgboost")) %>%
...
wflw_fit_nnet <- workflow() %>%
  add_model(mlp(mode = "regression") %>% set_engine("nnet")) %>%
  add_recipe(recipe_spec %>% update_role(date, new_role = "indicator")) %>%
  fit(train_cleaned)
...
tune_results_earth %>% show_best("rmse", n = Inf)
...
#model_ensemble_refit_tbl$.model[[1]]$model_tbl %>%
...
# Looking at individual models in ensemble
model_ensemble_refit_tbl$.model[[1]]$model_tbl
