...
full_data_tbl <- ga_page_raw_tbl %>%
  ...
  pad_by_time(
    ...
    .by         = "day",
    .pad_value  = 0,
    .start_date = min(ga_page_raw_tbl$date)) %>%
  ungroup() %>%
...
tune_results_xgboost <- tune_grid(
  ...
  param_info = parameters(wflw_spec_tune_xgboost) %>%
    ...
    update(mtry       = mtry(range =c(1L, 60L), trans = NULL)),
  grid       = 10,
...
wflw_fit_xgboost <- wflw_spec_tune_xgboost %>%
  finalize_workflow(best_results %>% slice(1)) %>%
  fit(training(splits))
...
ensemble_reloaded_tbl <- modeltime_table(
  ...
  ensemble_weighted(loadings = c(3, 3, 7)) %>%
  modeltime_table()
