...
wflw_fit_mars_simple <- workflow() %>%
  ...
  add_recipe(
    recipe = recipe(optins_trans ~ optin_time,
      data = training(splits) ) %>%
      step_mutate(optin_time = as.numeric(optin_time))
    ) %>%
  fit(training(splits))
...
simple_fit_glmnet <- model_spec_glmnet_simple %>% fit(value ~ as.numeric(date) + quarter(date), sample_data_tbl)
