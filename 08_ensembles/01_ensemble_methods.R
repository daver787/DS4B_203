...
ensemble_fit_wt <- submodels_tbl %>%
  ensemble_weighted(
    loadings = loadings_tbl %>% pull(rank)
  )
...
model_stack_level_3_tbl <- modeltime_table(
  ...
  ensemble_weighted(loadings = c(3, 2, 1)) %>%
  modeltime_table()
...
# Not updating the super learner / updating the submodels only

refit_ensemble_submodel_tbl <- calibration_ensemble_tbl %>%
  modeltime_refit(data = data_prepared_tbl)

forecasts_submodel_tbl <- refit_ensemble_submodel_tbl %>%
  modeltime_forecast(
    new_data    = artifacts_list$data$forecast_tbl,
    actual_data = data_prepared_tbl
  )

forecasts_submodel_tbl %>%
  plot_modeltime_forecast()

# Updating the super learner and the submodels
set.seed(123)
...
model_stack_level_3_tbl$.model[[1]]$model_tbl$.model[[2]]$model_tbl$.model[[1]]
