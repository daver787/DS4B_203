...
transactions_trans_weekly_tbl <- transactions_weekly_tbl %>%
  ...
  mutate(revenue_trans = standardize_vec(revenue_log))
...
4. __Add rolling features from our lag__: Use `tk_augment_slidify()` to add `.period = rolling_periods`. Use `mean` as the rolling function. Make sure to "center" with "partial" windows. 
5. __Add events__:
...
full_tbl <- transactions_trans_weekly_tbl %>%
  ...
  mutate(across(starts_with("event"), .fns = ~replace_na(.x, 0))) %>% 
  rename_with(.cols = contains("lag"), .fn = ~str_c("lag_", .)) %>%
  select(-revenue, -revenue_log)
...
```{r}
# YOUR CODE HERE
recipe_spec_base <- recipe(revenue_trans ~ ., data = training(splits)) %>%
...
data_prepared_tbl %>%
  plot_time_series_regression(
    .date_var = purchased_at,
    .formula  = revenue_trans ~ splines::ns(x = purchased_at, df = 3)
    )
```
...
recipe_spec_1_spline <- recipe_spec_base %>%
  step_rm(purchased_at,starts_with("lag_")) %>%
  step_ns(purchased_at_index.num, deg_free = 3)
...
Create a recipe for the lag model. 

1. Start with `recipe_spec_base`
2. Add a step to remove the "purchased_at" feature. We don't need this for LM models. 
3. Remove missing values in any column that starts with "lag_"
4. Store your updated recipe as `recipe_spec_1_lag`
...
```{r}
# YOUR CODE HERE
model_tbl <- modeltime_table(workflow_fit_lm_2_lag, workflow_fit_lm_1_spline)
...
refit_tbl %>%
  ...
  mutate(across(.value:.conf_hi, ~exp(.))) %>%
  plot_modeltime_forecast()
```
...
```{r}
worflow_fit_glmnet_2_lag <- workflow_fit_lm_2_lag %>%
  update_model(
    spec = linear_reg(mode = "regression", penalty = 0.1, mixture = 0.5) %>%
      set_engine("glmnet")
  ) %>%
  fit(data = training(splits))
```
...
calibration_tbl <- modeltime_table(
  ...
  worflow_fit_glmnet_2_lag
) %>%
  update_model_description(.model_id = 1, "LM - Spline Recipe") %>%
  update_model_description(.model_id = 2, "LM - Lag Recipe") %>%
  update_model_description(.model_id = 3, "GLMNET - Lag Recipe") %>%
